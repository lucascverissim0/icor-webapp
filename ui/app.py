...
st.markdown(
    ...
    .card { background: #161A22; border: 1px solid #30363d; border-radius: 16px; padding: 18px; text-align: center; transition: transform 0.08s ease-in-out; cursor: pointer; }
...
def main():
    ...
    with col1:
        st.markdown(
            '<div class="card"><div class="card-emoji">🚗</div>'
            '<div class="card-title">Fleet by Model / Year</div>'
            '<div class="subtle">View EU or World</div></div>',
            unsafe_allow_html=True,
        )
        if st.button("Open • Fleet by Model / Year", key="open_fleet", use_container_width=True):
            st.session_state["section"] = "fleet"

    with col2:
        st.markdown(
            '<div class="card"><div class="card-emoji">🛠️</div>'
            '<div class="card-title">Windshield Replacements</div>'
            '<div class="subtle">View EU or World</div></div>',
            unsafe_allow_html=True,
        )
        if st.button("Open • Windshield Replacements", key="open_repl", use_container_width=True):
    ...
    with col3:
        st.markdown(
            ...
            '<div class="subtle">Pick EU/World & Year</div></div>',
            unsafe_allow_html=True,
        )
        if st.button("Open • Historical Sales", key="open_hist", use_container_width=True):
    ...
    if section == "fleet":
        ...
        if not sheet_exists(sheet_name):
            ...
            try:
                nice_table(pd.read_excel(EXCEL_PATH, sheet_name=sheet_name))
    ...
    # =============== SECTION: HISTORY ===============
        ...
        if not years:
            ...
            try:
                nice_table(pd.read_excel(EXCEL_PATH, sheet_name=sheet_name))
                track("view_history", {"region": region, "year": int(year)})
            except Exception as e:
                st.error(f"Could not read {sheet_name}: {e}")

# ---------- Hard crash guard at import time ----------
try:
    main()
except Exception as e:
