  ...
  return (
    <article className="configuration-detail">
      ...
      <header className="detail-hero">
        <div>
          <p className="eyebrow">Generation opportunity detail</p>
      ...
      <dl className="identity-strip">
        <div><dt>Generation ID</dt><dd>{configuration.generation_id ?? 'Unavailable'}</dd></div>
        <div><dt>Identity kind</dt><dd>{configuration.generation_identity_kind ? titleCase(configuration.generation_identity_kind.replaceAll('_', ' ')) : 'Unavailable'}</dd></div>
      ...
      <section className="detail-section" aria-labelledby="demand-heading">
        <h2 id="demand-heading">Opportunity composition</h2>
        <div className="demand-highlight">
          <div><span>P50 opportunity</span><strong>{formatUnits(configuration.demand.base_units)}</strong></div>
        ...
        <p className="detail-note">This baseline is generation-level and does not claim an exact windshield, trim, or ADAS fitment. Assumptions: {(configuration.assumption_ids ?? []).join(', ') || 'none supplied'}.</p>
      </section>
      ...
      <section className="detail-section" aria-labelledby="sources-heading">
        <h2 id="sources-heading">Official source releases</h2>
        <div className="source-list">
          {configuration.sources.map((source) => <article key={`${source.name}-${source.description}`}><h3>{source.name}</h3><p>{source.description}</p><span>Snapshot evidence</span></article>)}
        </div>
      </section>
...
export function ConfigurationDetail({ apiClient = plannerApi, backHref, configurationId, onBack }: ConfigurationDetailProps) {
  ...
  if (detailQuery.isError) {
    ...
    if (notFound) return <section className="planner-state"><p className="eyebrow">No matching generation</p><h1>Opportunity not found</h1><p>The requested identifier is unavailable in the active snapshot.</p><ReturnAction backHref={backHref} label="Return to planner" onBack={onBack} primary /></section>
